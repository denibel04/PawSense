...
export class ReportService {
    ...
    generateReportFromAudio(file: Blob, filename: string, reportType: 'veterinario' | 'adiestramiento'): Observable<SSEEvent> {
        return new Observable(observer => {
            ...
            console.log(`[DEBUG] ReportService: Enviando request a: ${apiUrl}`);
            console.log(`[DEBUG] ReportService: Tipo de reporte: ${reportType}`);
            console.log(`[DEBUG] ReportService: Archivo: ${filename}`);
            ...
            .then(response => {
                ...
                const processChunk = async () => {
                    try {
                        ...
                        if (done) {
                            console.log(`[DEBUG] ReportService: Stream completed. Total events: ${eventCount}`);
                            observer.complete();
                            return;
                        }
                        ...
                        lines.forEach(line => {
                            if (line.startsWith('data: ')) {
                                try {
                                    ...
                                    console.log(`[DEBUG] Event #${eventCount}:`, eventData.status, `(${eventData.percent}%)`);
                                    
                                    observer.next(eventData);

                                    // Si está completado o hay error, completar observable
                                    if (eventData.completed === true) {
                                        console.log(`[DEBUG] Event marked as completed`);
                                        setTimeout(() => {
                                            observer.complete();
                                        }, 100);
                                    } else if (eventData.error === true) {
                                        console.error(`[DEBUG] Event marked as error:`, eventData.message);
                                        setTimeout(() => {
                                            observer.error(new Error(eventData.message));
                                        }, 100);
                                    }
                                } catch (e) {
                                    console.debug('Error parsing SSE event:', e, 'Line:', line);
            ...
            return () => {
                console.log('[DEBUG] ReportService: Unsubscribing');
